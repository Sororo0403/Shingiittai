...
float3 ApplyDamageVignette(float3 color, float2 uv)
{
    float amount = saturate(damageVignetteStrength);
    if (amount <= 0.0001f)
    {
        return color;
    }
...
float3 ApplyParryVignette(float3 color, float2 uv)
{
    float amount = saturate(parryVignetteStrength);
    if (amount <= 0.0001f)
    {
        return color;
    }
    ...
    float diagonal =
        pow(saturate(1.0f - abs(centered.x + centered.y) * 0.62f), 2.0f);
    float mask =
        saturate(edge * 0.52f + corner * 0.38f + diagonal * edge * 0.34f);
...
bool IsPostProcessBypass()
{
    return colorMode == 0 &&
           ...
           damageVignetteStrength <= 0.0f &&
           parryVignetteStrength <= 0.0f;
}
...
float4 main(PostProcessVSOutput input) : SV_TARGET
{
    ...
    outputColor.rgb = ApplyDamageVignette(outputColor.rgb, input.uv);
    outputColor.rgb = ApplyParryVignette(outputColor.rgb, input.uv);
