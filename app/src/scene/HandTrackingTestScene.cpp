...
HandTrackingTestScene::~HandTrackingTestScene() { ClosePreviewSocket(); }
...
void HandTrackingTestScene::Update() {
    ...
    const bool hasHandData = handController_.HasRecentPacket();
    ...
    if (ctx_->input->IsKeyTrigger(DIK_TAB) ||
        ctx_->input->IsKeyTrigger(DIK_BACK)) {
        sceneManager_->ChangeScene(std::make_unique<TitleScene>());
        return;
    }
    if (ctx_->input->IsKeyTrigger(DIK_F1)) {
        cameraRequestTimer_ = 999.0f;
        handTrackingStartRequested_ = false;
        RequestHandTrackingStartOnce();
    }
    if (ctx_->input->IsKeyTrigger(DIK_C)) {
        CalibrateNeutralFromCurrentHands();
    }
    if (ctx_->input->IsKeyTrigger(DIK_R)) {
        ResetCalibration();
    }
}
...
void HandTrackingTestScene::UpdateHand(size_t handIndex) {
    ...
    debug.active = handController_.IsActive(handIndex);
    debug.hasCenter = handController_.GetHandCenter(handIndex, debug.x, debug.y);
    debug.rawSpeed = handController_.GetRawMotionSpeed(handIndex);
    debug.motionSpeed = handController_.GetMotionSpeed(handIndex);
    ...
    if (!wasSlash && debug.slash) {
        ...
        debug.pulse = (std::max)(0.0f, debug.pulse - ctx_->deltaTime * 3.6f);
    }
...
void HandTrackingTestScene::CalibrateNeutralFromCurrentHands() {
    ...
    for (size_t i = 0; i < hands_.size(); ++i) {
        ...
        if (handController_.GetHandCenter(i, x, y)) {
            calibration_.handNeutral[i] = {x, y};
            hasNeutral = true;
        }
    }
...
void HandTrackingTestScene::DrawCameraBadge(float, float) {
    const bool cameraAvailable = !ctx_->isCameraDeviceAvailable ||
                                 ctx_->isCameraDeviceAvailable();
    const bool ready = IsHandTrackingReady();
    const bool hasPacket = handController_.HasRecentPacket();
    const float x = 20.0f;
    const float y = 18.0f;
