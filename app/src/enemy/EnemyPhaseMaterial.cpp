...
EnemyPhaseMaterialProfile
LerpEnemyPhaseProfile(const EnemyPhaseMaterialProfile &from,
                      const EnemyPhaseMaterialProfile &to, float t) {
    t = SmoothStep01(t);
    EnemyPhaseMaterialProfile profile = to;
    profile.reflection =
        from.reflection + (to.reflection - from.reflection) * t;
    ...
    const size_t paletteSize =
        (std::max)(from.palette.size(), to.palette.size());
    profile.palette.clear();
    profile.palette.reserve(paletteSize);
    for (size_t i = 0; i < paletteSize; ++i) {
        const XMFLOAT4 fromColor = from.palette.empty()
                                       ? XMFLOAT4{1.0f, 1.0f, 1.0f, 1.0f}
                                       : from.palette[i % from.palette.size()];
        const XMFLOAT4 toColor = to.palette.empty()
                                     ? XMFLOAT4{1.0f, 1.0f, 1.0f, 1.0f}
...
void ApplyEnemyPhaseMaterialProfile(ModelManager *modelManager,
                                    uint32_t modelId,
...
void InitializeEnemyPhaseMaterialSet(TextureManager *texture,
    ...
    materials.rustPixels = CreateProceduralTexturePixels(
        size, size, {0.23f, 0.22f, 0.20f}, {0.70f, 0.30f, 0.12f}, 0x914Au,
        0.62f, true);
    materials.cleanMetalPixels = CreateSmoothMetalTexturePixels(
        size, size, {0.31f, 0.33f, 0.32f}, {0.50f, 0.51f, 0.46f});
    materials.goldMetalPixels = CreateProceduralTexturePixels(
        size, size, {0.31f, 0.22f, 0.08f}, {0.56f, 0.41f, 0.16f}, 0xB05Du,
        0.38f, true);
    ...
    materials.goldMetalTextureId = texture->CreateFromRgbaPixels(
        size, size, materials.goldMetalPixels.data());
...
void ApplyEnemyPhaseMaterial(ModelManager *modelManager,
    ...
    const EnemyPhaseMaterialProfile rustProfile{materials.rustTextureId,
    ...
    const EnemyPhaseMaterialProfile cleanMetalProfile{
        ...
        0.12f,
        0.38f};
    const EnemyPhaseMaterialProfile goldProfile{materials.goldMetalTextureId,
    ...
    if (blendFromPixels != nullptr && blendToPixels != nullptr &&
        ...
        BlendTexturePixels(*blendFromPixels, *blendToPixels,
                           materials.blendPixels,
                           SmoothStep01(transitionRatio));
        textureManager->UpdateTexture2D(
            materials.blendTextureId, materials.blendPixels.data(),
            static_cast<size_t>(EnemyPhaseMaterialSet::kTextureSize) * 4u);
    }
